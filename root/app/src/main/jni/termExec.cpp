...
static int create_subprocess(const char *cmd, const char *arg0, const char *arg1,
    int* pProcessId)
{
    LOGI("create_subprocess1");
    char *devname;
    int ptm;
    pid_t pid;

    ptm = open("/dev/ptmx", O_RDWR); // | O_NOCTTY);//创建虚拟终端
    if(ptm < 0){
    ...
    LOGI("create_subprocess2");
    if(pid == 0){
        LOGI("create_subprocess3.1");
        close(ptm);

        int pts;
        LOGI("create_subprocess3.2");
        setsid();
        
        pts = open(devname, O_RDWR);
        if(pts < 0) exit(-1);
        LOGI("create_subprocess3.3");
        dup2(pts, 0);
        dup2(pts, 1);
        dup2(pts, 2);
        LOGI("create_subprocess3.4");
        execl(cmd, cmd, arg0, arg1, NULL);//    "/system/bin/sh", "-", null, processId
        exit(-1);
    } else {
        LOGI("create_subprocess4");
        *pProcessId = (int) pid;
        LOGI("create_subprocess5");
        return ptm;
    }
}
    ...
    jstring cmd, jstring arg0, jstring arg1, jintArray processIdArray)
{
    ...
    LOGE("android_os_Exec_createSubProcess1");
    jobject result = env->NewObject(class_fileDescriptor, method_fileDescriptor_init);
    LOGE("android_os_Exec_createSubProcess2");
    if (!result) {
...
static const char *classPathName = "androidterm/Exec";
